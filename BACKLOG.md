...
| B9 | **Rule overlap audit — 3 true contradictions + 1 tie need a decision** | Found 2026-07-18 via the new `find_overlapping_rules()` audit (part of the classification-engine redesign, see PLAN.md). Of 20 overlapping-pattern pairs found: 8 are "general pattern accidentally beats a more specific one by a priority-number coincidence" bugs that the redesign's specificity-based rule ordering (in progress) will fix automatically — no action needed there. The remaining 4 won't self-resolve and need an explicit decision: **two `CHASE CC PAYMENT` rules** (id 14 → Transfer vs id 524 → Fees & Interest/Expense — 524 is currently fully shadowed/dead), **two `AUTOMATIC PAYMENT` rules** (id 528 → Transfer vs id 529 → Expense — 529 is dead), **`ALIGNED INCENTIV` vs `ALIGNED INCENTIVES`** (id 500 vs id 65 — looks like a typo variant, one shadows the other), and **`SCHEELS ALL SPORTS` vs `SCHEELS ALL SPORTS IFARGO`** (ids 522/523, identical priority 200 — currently resolved by non-deterministic Postgres row order, a real live risk). |
