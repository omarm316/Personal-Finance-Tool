...
| B6 | **`_check_transfer_correction()` over-generalizes learned corrections** | `categorization.py` — a single user correction (merchant→Transfer) gets blanket-applied to every transaction from that merchant. Its own anti-over-generalization safeguard (word-overlap between descriptions) fails when the correction's description has no words beyond the merchant name itself. Confirmed live: one 2026-03-04 Best Buy correction was silently flipping every Best Buy purchase to Transfer once `force_unlock=True` reprocessed locked rows (found + reverted 2026-07-18, see PLAN.md). Root cause not yet fixed — other merchants with a similarly generic single correction are at the same risk. |
| B7 | **Venmo mis-tagged "Healthcare" budget category** | A stray auto-created rule (`id=504`, "Auto-created from LLM enrichment") sets `set_category='Healthcare'` for any VENMO-pattern transaction — 5 rejections, 0 acceptances in its history, suggesting it's been silently wrong and manually corrected repeatedly. Found 2026-07-18 while adding the P2P Payments CSC; not fixed (different field than points_category, out of scope for that fix). |
