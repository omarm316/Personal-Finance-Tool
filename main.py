...
# both do `from database import SessionLocal` at their own top level, and
# that name must already be the real sessionmaker (not the `None` placeholder
# database.py defines before init_db() runs) by the time that import
# executes, or the module would bind the stale None permanently. The same
# applies to the core.plaid_sync import right below, for the same reason —
# main.py's own reset_all/reset_and_resync_account call _sync_item_background
# directly and would otherwise get a permanently-None SessionLocal inside it.
# ---------------------------------------------------------------------------
